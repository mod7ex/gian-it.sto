        ...
        <nav class="-mb-px flex space-x-6" aria-label="Tabs">
          <div v-for="(tab, index) in items" :key="tab.label" class="flex">
            <router-link
              ...
              :class="[tab.current ? 'cursor-pointer border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'cursor-pointer whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm']"
            >
              {{ tab.label }}
            </router-link>
