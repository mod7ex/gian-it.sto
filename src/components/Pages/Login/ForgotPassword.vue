...
import useForgotPassword from '~/services/auth/forgotPassword.js';

const { refreshPassword, v$, data, form, loading, success, errorMsg, ready } = useForgotPassword();
</script>
    ...
    <LoginLayout :title="data?.message ?? 'Забыли пароль?'">
      <div class="mt-8">
        <div class="mt-6">
          ...
          <p v-if="success && ready" class="text-green-700 mb-6">{{ form.email }}</p>
          <p v-if="!success && ready" class="text-red-500 text-sm mb-6">{{ errorMsg ?? 'Undefined (network?) error' }}</p>
          ...
          <form class="space-y-6" @submit.prevent="refreshPassword">
            ...
            v-model="v$.email.$model" v-if="!ready" :error="(v$.email.$error) ? v$.email.$silentErrors[0].$message : ''" />

            <Button color="blue" class="w-full justify-center" @click.prevent="refreshPassword" :disabled="loading"
            :class="{ 'cursor-not-allowed': loading, 'opacity-60': loading }" v-if="!ready">
